...
class ShipRepository:
    ...
    def create_ship(self, ship) -> models.ShipModel:
        db_ship = models.ShipModel(
            id=ship.id,
            title=ship.title,
    ...
    def get_by_id(self, ship_id: int) -> models.ShipModel:
        ship = self.db_session.execute(
            select(models.ShipModel).filter(models.ShipModel.id == ship_id)
        )
        return ship.scalars().first()

    def get_all_ships(self):
        ships = self.db_session.execute(select(models.ShipModel).order_by(models.ShipModel.id))
        return ships.scalars().all()

    def update_ship(self, ship) -> None:
        ship_update = update(models.ShipModel).where(
            models.ShipModel.id == ship.id
        ).values(
            title=ship.title,
            fuel=ship.fuel,
            total_weight_capacity=ship.total_weight_capacity,
    ...
    def delete_ship(self, ship_id: int) -> None:
        ship_delete = delete(models.ShipModel).where(
            models.ShipModel.id == ship_id
        )
