...
class Ship(IShip):
    def __init__(self, port, ID, data_ship: DataShip, fuel: float = 0.0):
        self.id = ID
        self.fuel = fuel
        self.port = port
        self.data = data_ship
    ...
    def load(self, container):
        if len(self.containers) < self.data.maxNumberOfAllContainers and self.weight+container.consumption() <= self.data.totalWeightCapacity and container in self.port.containers:
    ...
    @staticmethod
    def init_ship(value, port, ID, data_ship, fuel):
        if value <= 40000:
            return LightShip(port, ID, data_ship, fuel)

        elif 40000 < value < 90000:
            return MediumShip(port, ID, data_ship, fuel)

        else:
            return HeavyShip(port, ID, data_ship, fuel)


class LightShip(Ship):
    def __init__(self, port, ID, data_ship, fuel):
        super().__init__(port, ID, data_ship, fuel)
