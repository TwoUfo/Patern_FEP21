...
public class CreditCard
{
    ...
    private string Decrypt(string hashedValue)
    {
        // For comparison, we check if the hashes match
        if (hashedValue.Equals(_cvv, StringComparison.OrdinalIgnoreCase))
        {
            // If the hashes match, return the original value
            return hashedValue;
        }
        else
        {
            // If the hashes do not match, return some indication of failure
