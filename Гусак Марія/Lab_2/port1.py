...
from ship1 import ShipClass
from conteiner1 import *
...
class IPort(ABC):
    ...
    @abstractmethod
    def incomingShip(self, ship: ShipClass):
        pass

    @abstractmethod
    def outgoingShip(self, ship: ShipClass):
        pass
...
class PortClass(IPort):
    ...
    def incomingShip(self, ship: ShipClass) -> bool:
        if isinstance(ship, ShipClass) and ship not in self.currentShips:
    ...
    def outgoingShip(self, ship: ShipClass) -> bool:
        if isinstance(ship, ShipClass) and ship in self.currentShips:
