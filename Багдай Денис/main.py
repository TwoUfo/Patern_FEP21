...
class Customer:
    ...
    def talk(self, minutes: int, customer: 'Customer', operator_name: str):
        ...
        print(f"Customer {self.first_name} talked to {customer.first_name} for {minutes} minutes. Cost: {cost}")
    ...
    def message(self, quantity: int, customer: 'Customer', operator_name: str):
        ...
        self.bills[operator_name].add(cost)
        print(f"Customer {self.first_name} sent {quantity} messages to {customer.first_name}. Cost: {cost}")
    ...
    def connect(self, traffic: float, operator_name: str):
        ...
        if not self.bills[operator_name].check(network_cost):
            ...
            print(f"Customer {self.first_name} used {traffic} MB of data. Cost: {formatted_cost}")
        else:
...
class Bill:
    def __init__(self, limiting_amount: float = 100):
...
def main():
    ...
    for customer in customers:
        customer.talk(10, customers[0], "Operator1")
        customer.message(20, customers[1], "Operator2")
        customer.connect(50, "Operator3")
